...
# Find the nearest LIVING Hunter (survival mode, not us) relative to our position
# Execute as that Hunter, teleport the respawning one to them
execute at @s as @a[team=Hunter,tag=!mh.needsteleport,gamemode=survival,sort=nearest,limit=1] at @s run teleport @a[team=Hunter,tag=mh.needsteleport,limit=1] @s
